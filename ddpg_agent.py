...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done, agent_number):
        """ Save experiences in the replay memory buffer and use random samples from the buffer to learn """
        self.timestep += 1
        # Save the experience and reward
        self.memory.add(state, action, reward, next_state, done)
        # If the samples are bigger than predefined BATCH_SIZE; start to learn at the LEARN_EVERY intervals
    ...
    def act(self, states, add_noise):
        """ Returns actions of the agents based on their current policies / states """
        states = torch.from_numpy(states).float().to(device)
        actions = np.zeros((self.num_agents, self.action_size))
        self.actor_local.eval()
        with torch.no_grad():
            # Get the actions for each agent and merge
    ...
    def learn(self, experiences, gamma, agent_number):
        ...
        # ----------------------- Update Target Actor-Critic Networks ----------------------- #
